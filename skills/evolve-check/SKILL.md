...
| T12 | **本轮花销信号**：收尾跑 `python3 scripts/audit_skill_cost.py --check` 报固定面涨了；或 `node scripts/session_cost.mjs --steps` 里出现重复读同一份、单次返回特别大、步数明显多于同类 | 花费在无回报处增长 | 固定面按查出的那一项收回去；收不回就改基线并在 PR 写明理由。流程面产 L2 卡记可疑步骤并附逐步表 |
