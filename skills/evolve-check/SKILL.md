...
| T12 | **本轮花销信号**：收尾跑 `python3 scripts/audit_skill_cost.py --check` 报固定面涨了；或 `node scripts/session_cost.mjs --tools` 显示调用次数与返回累计偏高、同一目标被反复访问 | 花费在无回报处增长 | 固定面按查出的那一项收回去；收不回就改基线并在 PR 写明理由。流程面产 L2 卡，附 `--tools`/`--steps` 读数并指明是次数问题还是目标重复 |
...
- **改进动作必须先产 EV 卡（前置元流程，防绕过）——范围限定为四类**：①**从数据推断**的归纳
  （T1 归纳 → to-reference --ingest-cases、T5 扩 reference 家族）；②改动**行为面**（triage 分支、
  skill 流程步骤、闸门绑定、表族追加——凡改变命中或加载行为的）；③**修订已合入的 active 词条**；
  ④索引与口径类机制改动。四类都走 **产卡（proposal）→ 执行（action）→ 验证（eval）→ 判断（decision）
  → 才随 PR 合入**；禁止先执行后补卡（产出已生效而决策链缺失，人审无据）。
  **豁免：来源明确的常规参考知识导入**（新增词条、不改行为面）——这类改动没有可证伪的预测，
  决策记录由 reference 类 PR 模板承载（变更类型 / 词条清单 / 来源与验证状态 / 聚类检查 / 预核意见），
  不再要求 EV 卡；`to-reference` 收尾照常跑本协议，**按触发条件表有真信号才产卡**。
  **为什么豁免**：给每条导入配卡只会产出"预测＝CI 能过"的仪式卡，与本协议"预测必须可复现"的
  要求自相矛盾（原则四/十：不为产卡而产卡）。内容流程产出草稿进 inbox（待审队列）同样不在此列。
