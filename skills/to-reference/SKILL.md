...
**写词条时的行文**：reference 词条是给人检索、给 agent 载入的文本，按 `docs/spec/writing-norms.md` 写（可选论证层，不影响本 skill 执行）；本面的定制条款见该文件 §3 的「case / reference 词条」一行——症状句要能直接当 grep 判据（保留报错原文与算子名），事实与结论直接写，不写推理过程。
