...
3.5. **triage 路由同步（知识增长自动补全路由）**：产出 case 草稿后，检查该 case 的 `symptoms` 关键词能否被路由正则（`triage-tree.yaml`，生成物）路由到正确 namespace。**加词改的是源**：`triage-tree.d/<族>.yaml`（一族一文件，如 `40-inference-interrupt.yaml`），改完跑 `python3 scripts/build_triage_tree.py`——聚合的 `triage-tree.yaml` 由主干重建，不要提交它（CI 有门拦）：
   - 能 → 无需动作（路由已覆盖）；
