...
1. **收集症状 + 确认框架**（全部来自工程师提供）：**先提取、再问缺口**——错误信息 / 环境变量值 / 版本组合优先从已提供的日志（栈尾、启动横幅、配置）里提，**只问提取不到的那几项**，并把提出来的值列给他核对一次；**信息不全就主动问**；**主动裁剪日志**（失败 rank + 栈尾，绝不灌全量 profiler）。→ 展开见 reference 步骤 1。
