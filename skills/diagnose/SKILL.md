...
1. **收集症状 + 确认框架 + 落负载类型**（全部来自工程师提供）：本步产出一张**覆盖表**（行固定：错误原文 / 触发位置 / **负载类型（训练 / 推理）** / 引擎 / CANN / HDK / 平台 / 必现偶发 / 单机多机 / 已试过什么）——**先提取、再问缺口**：值优先从已提供的日志（栈尾、启动横幅、plog、配置）里提，**提取不到的行留空即待问项**，只问空着的那几行、一次问全；**每个值标来源**（日志 / 命令输出 / 配置 / 他的描述——凡"来自他的描述"，涉及版本与平台时要拿命令输出复核），报的时候只报确认行与待补项、按主题分组逐项可核（不复述原文）；**信息不全就主动问**；**主动裁剪日志**（失败 rank + 栈尾，绝不灌全量 profiler）。**负载类型决定去库里哪一半找，来源比值还重要**：材料里写着（训练任务 / 推理请求 / `vllm serve` 这类服务命令）就直接用；没写就**跟引擎版本并列问一句**（他一定知道，别让模型去猜）；他没单说但给了框架名，就直接对到库里目录（`knowledge/training/<框架>/` 还是 `knowledge/inference/<框架>/`）；他一时不答就按负载类型未知走（训练与推理都查，trace 标 `workload_type: unknown`），不阻塞；中途证据与他的说法相反时**以证据为准并回一句**让他能改向。→ 展开见 reference 步骤 1。
2. **分类 → `triage-tree.yaml`（Tier 1）**：**先落负载类型、再在负载类型内判性质**——`workload_types:` 给合法负载类型与每种负载类型目录面（负载类型来自步骤 1 的事实，不由症状词判），`natures:` 给三个性质分支（interrupt / precision / performance，训推共用一份词表），症状匹配只判性质、不判负载类型；triage 决策记 trace（**`workload_type` 与 `workload_type_source` 必写**，`workload_type_source` 取 evidence / user / framework / unknown；**`triage` 事件必须带 `routed`**，没命中就写 `routed: []` 而不是省略字段——省略与「路由没错」在指标上同形）；未命中 → 语义兜底 `triage_semantic`（带 namespace）；无法分类 → Tier 3。**收尾做键触发**：证据里的错误码 / 故障签名 / 环境变量名 / 版本组合当场查先验查表族（**先于候选加载**，每次必留一条 trace，含"没有可查的键"）；**错误码在族表里没有行 → 查 `references/errors/_code-gaps.yaml`（缺行索引，`seen_in` 直接给到有修法的那张表），再报"没有"**。→ 展开见 reference 步骤 2。
...
| 判断缺口（理解方面） | 证据里有错误码 / 故障签名 / 环境变量名 / 版本组合 | **步骤 2 收尾，先于候选加载** | `purpose: signature` |
| 判断缺口（背景方面） | 候选命中 | 步骤 3 阶段 2.5 | `purpose: background\|fix` |
...
- **类别冲突不得二选一（但有先后）**：日志/报错签名指向的方向与**症状性质**冲突时（例：症状是"输出乱码"→ precision，日志却是 store/BM 初始化失败 → interrupt 味），**按症状的性质定 category——症状是工程师手上那个现象，日志签名只是线索**；自动单选先走症状那条线，**在同一屏里说明另一条线的存在**（它需要哪些信息才走得通）。给一句覆盖点：「若你更关心 X，说一声我就切过去」——**不要停下来问他"你觉得这是哪类问题"**：他答不了这个判断，那正是要找你的原因。日志签名一负载类型的证据不丢：它作为备选线写明换向条件，不因为当前没走就删掉。
...
- **知识库版本**：建 session 时写顶层 `kb_rev`（`python3 scripts/kb_rev.py` 的末行 = 检出 HEAD 短 sha），**一次写定、之后不改**。它记的是"这一单跑在哪一版 `knowledge/` 上"：跨机接手时接手方要拿它比对本机版本（不一致意味着候选集与 case id 可能对不上，接手负载类型会看到提示），事后误诊归因也要它（"当时为什么没命中"取决于当时库里有什么）。拿不到 git 就写 `unknown`，别编值。
...
1. **先定"哪个仓"，再向其确认版本**（源码分析依赖对应版本，不要猜）：
   ...
   - 已知表里没有 → **枚举组织仓给候选，不要直接问人要地址**：`python3 scripts/src_fetch.py --find "<报错签名>" --ref <版本>`（命令与退出码展开见 `references/src-fetch.md`）。脚本按签名里的接口前缀与算子名给带理由的候选（默认 3 个），并**逐个试该版本**：只有一个候选能解析就自动取回（exit 0）；多个就 exit 3 交回候选表。候选 ≥2 时按闸表 `source-location` 的那一句问工程师（"这个报错在你们那的 CANN 安装目录里落在哪个子目录？"，让他回候选编号）——**别猜仓**：取错仓产出的是错版本的证据，比不取更糟。CANN 各仓共用同一套 CANN 发布号，所以"tag 能解析"不构成定位证据。
