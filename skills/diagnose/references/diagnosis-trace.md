...
**字段齐全不等于读得懂**：面板把顶层 `summary` 与逐事件的 `output`/`reason` 直接摊给读者，所以它们与报告正文适用同一套行文纪律。要点（与面板文案规范同源，不另立一套）：
...
- **不用比喻与拟人、不用对偶排比、不用强调副词、不自我评价**。"如实记录""不冒充"一类一律删——如实是默认要求，不必在正文里声明。
- **例外：证据原值不翻译**。case/issue 编号、`文件:行号`、命令、数字、报错原文照抄——读者要能拿它去复核，通俗化会损失可核查性。
