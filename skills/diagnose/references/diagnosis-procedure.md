...
**负载类型（训练 / 推理）＝ 路由的第一步**，它决定去库里哪一半找，所以本行的来源要连同值一起记：
  ...
  ③ 他没单说，但顺手给了框架名 → 直接对到库里目录：`knowledge/training/<框架>/` 还是
     `knowledge/inference/<框架>/`。对得上就用，来源记 `框架`（这就是"侧"的第三档来源，
     不是另建一张表——库里的目录本身就是那张表，新框架来了照着放就行）
  ④ 他一时不答 → **侧未知不是阻塞项**：两侧都查（`common/` 两侧都查照旧），
     在 trace 里写 `side: unknown`、`side_source: unknown`，照常往下走
  ⑤ 中途证据与他的说法相反 → **以证据为准**，并回一句让他能改向
     （例：「你说训练，日志里是 `vllm serve`——我按推理走；如果确实是训练请纠我」），
     来源记 `证据`，别把矛盾咽掉
  别做的事：不要从"训练集大小""grad 相关字样"这类间接痕迹推侧；也不要因为拿不准就把
...
## 步骤 2：分类 → triage-tree（先落侧，再在侧内判性质）

加载 `triage-tree.yaml`。它给两样东西：`sides:`（合法侧 + 每侧的目录面与检索顺序）与
`natures:`（三个性质分支：`interrupt` / `precision` / `performance`，各带 `category` 与症状正则）。
**侧与性质不互相竞争**：侧由步骤 1 的负载类型那一行确定（材料 / 问他 / 框架名，四档见步骤 1），
性质才由症状匹配判定。所以顺序是：先按侧取出候选目录面，再在**这一侧内**匹配性质分支。

**分批读取别整份灌进来**：三个性质的症状组是训推共用的（同一份词表服务两侧），
所以"看哪个性质"这一步读的就是 `natures:` 那一份正则；`sides:` 是几行常量。
这是分层的直接好处——原先同一个宽词在训推两侧各写一遍（全库实测 57 条），
分支顺序一决定，训练分支就会把推理侧的 case 先接住（全库 72 条），现在这种碰撞结构上不存在。

**triage 决策必记 trace**（`side` 与 `side_source` 是分层后新增的两个字段，**每次都要写**）：
```yaml
...
```
- `side`：`training` / `inference`，或 `unknown`（他没答、材料也没写——那就不猜）
- `side_source`：`evidence`（材料里读到的）｜`user`（问他、他答的）｜`framework`（对到了库里目录）｜`unknown`（只有 `side: unknown` 时才用它——四档词表与判定见 `diagnosis-trace.md`）
- `branch` 现在是**性质**名（`interrupt` / `precision` / `performance`），不再带 `training_` / `inference_` 前缀
  ——侧已经在 `side` 里，分支名再带一次就是两处记同一件事。分层前写下的历史 trace 保持原样，不改写。
- `routed` 记的仍是 namespace 列表（**带侧前缀**，如 `["inference/vllm-ascend/", "common/"]`）——
  路由准确率的分母取它，所以侧在 `routed` 里必须看得到。

路由规则：
- 侧已定 → 取该侧三个性质分支；框架检测到 → `search_namespaces` 先 `<side>/<framework>/`，再 `common/`
- 侧已定但框架未检测到 → 只 `common/`
- **侧未知** → 两侧都查（`<side>/` 展开成两个侧），trace 写 `side: unknown`。这不该是常态：
  按它的代价（候选面翻倍）在 trace 里记一笔，够频繁就说明步骤 1 那句问话没问出去。
- **冲突时先单选，不要求工程师表态**：日志签名与症状性质指向不同 category 时（详见 SKILL「始终要避的坑」），**自动单选**并在一句话里说清两条线——先走症状那条，日志那条写明它需要什么信息才走得通；给一句覆盖点让他能改向。**不要把"你觉得这是哪类问题"抛回去**：那是要他做他做不了的判断。两条线的按需参考入口各走各的（精度 / 性能走数据探询闸门，中断走条件型闸门），不因为先走一条就跳过另一条的记录。注意这里的"冲突"是**性质**之间的（同一侧内部），不是侧之间的——侧不需要在两个候选之间权衡，它是事实。
- **优雅退化**：多个性质弱匹配 / 置信度低 → 加载**该侧所有 namespace 的索引**让 quickly_check 筛（索引便宜，退化最坏 ~20K token 仍可控）；测不出侧时再退化到两侧都查。这救冷启动——triage-tree 第一周是猜的。
