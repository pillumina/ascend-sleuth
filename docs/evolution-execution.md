...
**统一框架：把每条 proposal、每次沉淀当成一次带预期的实验。** 执行前写下可测的 `predicted_effect`/`predicted_value`，执行后进入 follow-up 观察窗，对照预期判定（达标 / 再迭代 / 回滚），判定结果写回卡、指标、归因事件（按需聚合）。合理性不是合入那一刻的静态判断，而是"预期 → 实测"对照的动态闭环。
...
**2026-09 selfevolve-loop 重构**：原设计（predicted_value / first_hit / expected_window 字段 + 观察窗实验）从未落地——字段未进 case schema，且整套依赖 S1 真实反馈（断供中即空转）。第一性压缩：一次沉淀的效果由**两个现成通道**度量，不需要中间字段：
...
**resolve 数据源限定（保留核心语义）**：现场有效（resolve）**只认 S1**，落 `confidence.hits/mis`。S2/golden 证明的是"内容与外部 ground truth 一致 / 检索命中"，不是"fix 在现场解决"——两条证据不同对象，分别结算、不混算：
...
**与 verification 的分层（内容置信 ≠ 现场置信，保留）**：verification（来源验证：fix PR 合入等）提高的是**内容正确性先验**；validation_record 是它的**运行期延续**（持续外部验证累积）；confidence 是**现场解决率**（只 S1）。三者独立：fix-merged case 初始 score 高（内容可信），不等于它在任意客户环境都被验证过——现场确认仍需 S1 回报。
...
**反馈到源头（保留）**：某 issue 源（如 vllm-ascend 池）的沉淀连续不被验证或从未命中 → 调 `issue_filter.py` 的价值启发式，**而不是继续堆沉淀**——这使"沉淀质量"从 to-postmortem 环节延伸到 issue-ingest 的筛选环节（原则十一：数据回流到假设）。
...
（rejected）基于合入前可得的验证（S2/golden/归因事件复测）。**攒批、提 PR、人审合入是流程层**
...
actual_cost 成本字段在那边定义（2026-09 删 estimated_cost），本文不重复定义只引用——防两处状态机再次漂移。
...
**归因事件即决策记忆**：归因事件聚合 + proposals/reviews 历史 = agent 的"这个组件以前怎么改、结果如何"的记忆（对应 SkillOpt 的 reject buffer / meta-skill 思路，但载体是本仓库的词法归因事件 + 按需聚合，不是模型内隐状态）。
