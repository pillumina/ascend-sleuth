...
与用户指令的对应：这条指令即第一个长期任务——**先拉 200 个 issue（步骤 1 的扩池），前几轮的真实产出是"第一批落地"（建 S2、接执行日志、跑通任务循环——让机制第一次真实跑起来），之后才进入持续的沉淀与演进**。诚实边界不变：内容层（补 case/沉淀）可高度自动，结构层（triage/skill/指标口径）永远人审。
