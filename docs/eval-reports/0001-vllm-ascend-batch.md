...
### 后续批次的数据观察（0002 补充，2026-W35）

第二、三批继续从 vllm-ascend 抓取（增量游标 + 已处理编号去重，ADR-0006）：最新 60 候选通过率 18%（11/60）、更早 51 候选通过率 14%（7/51），均低于第一批的 33%。主因是 stale 机器人自动关闭占比高（wait-feedback→stale→not_planned，无 root cause/fix 确认）及 PR/文档/CI 线程混入。

**记录为数据点，不升级为强信号**：单仓库通过率波动受维护节奏影响大，样本仍小，不足以断言"issue 作为知识源边际收益递减"。若后续多批持续 <15% 且多个知识源同向，才考虑调整过滤门槛（如提高评论数门槛）或扩展知识源——届时按 roadmap 闸门驱动。
