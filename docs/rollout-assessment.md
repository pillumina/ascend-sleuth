...
> 评估日期：2026-08-28。评估基线：main HEAD（ADR-0008 全部阶段 + skill 修复 + 23 条 reference 已合入）。
...
| 二 不变量写进结构 | `build_index.py --check` CI、`verify_references.py` 三层强校验、`feedback_pending` 状态标记、机器路径 CI 挡（ADR-0008 phase 4 落地）、词条零注释规则 | ✅ 完整 |
| 三 底座保持词法 | 无向量检索（ADR-0002）、YAML/git 底座、quickly_check 双 regex | ✅ 完整 |
| 四 知识库有界 | soft_cap 30 / hard_cap 60 + 容量表、`_archive/` 软退休复活、方法论/资产分目录 | ⚠️ case 层完整；**reference 层容量治理参数仍是开放项**（ADR-0008 Open Question） |
| 五 建议与决定分离 | 预分诊三分类 + 人审、高风险双签 + 随机审序、to-reference grill→inbox→人审 | ✅ 完整 |
...
| 八 可观测先于改进 | trace 硬要求 + 固定词表、`trace_metrics.py`、`docs/metrics.md` | ⚠️ **机制完整但数据只有 W35 一期** |
| 九 稀缺资源显式预算 | 三层加载 + 日志裁剪、inbox 批处理 30 秒/条、reference summary 层先于全文、词条零注释、skill 去 ADR 锚定 | ✅ 完整且精细 |
| 十 诚实退化 | 空库提示三出路、Script 未接明说、机制强度如实标注、reference 只读 active、verification 状态、来源链诚实 | ✅ **体系最强项** |
| 十一 数据触发演进 | roadmap 闸门驱动、8 个 ADR 留痕、参数可复核 | ⚠️ 机制完整，**实测数据不足以支撑闸门** |
...
| reference 层（类型/状态/校验/CI） | ✅ 运行中（23 条 active） |
| 5 个 skill（diagnose / to-postmortem / to-reference / knowledge-groom / resume-diagnosis） | ✅ 全部有实现，to-reference 经 dogfood 修复 5 问题 |
| trace + 反馈闭环 + 误诊归因 | ✅ 机制完整 |
...
| references/ | 23 条全 A5 platform-fact；**A2/A3 无**；error-code/tool/methodology 类型**全空** | 错误码、工具、方法论是最常用先验知识 |
...
| reference 命中统计 | `reference_lookup` trace 事件已加，但**统计尚未开始积累** |

### 运维层（未就绪 ⚠️）

| 项 | 现状 |
|---|---|
| groom 周批 | 机制完整，**无任何实际运行记录** |
| 双签 owner | CODEOWNERS.example 占位，领域 owner / 体系维护人角色未落实 |
| 反馈闭环 | 机制在，但 38 条 case confidence 全是初始值——**没人回报过 fix 结果** |
| to-reference 使用 | 仅 dogfood 一次，无真实工程师使用 |
...
| P0 | groom 实际跑 2-3 轮（含 reference inbox 批处理），产出 metrics 二期 | 数据 + 运维（机制首次通电） |
| P1 | 补一个 training namespace 种子（mindspeed-llm 或 verl，10-20 条） | 内容（否则 training 团队无法使用） |
| P1 | plog 错误码表 + CANN 兼容矩阵进 reference（error-code 类首批） | 内容（最常用先验知识空白） |
...
- reference 层新增非 platform-fact 类型（error-code/tool/methodology 首个词条）；
