...
> **闸门驱动，不日期驱动。** 每个事项定义入口条件（数据或事件触发）与验收标准，解锁与否由 metrics 和 trace 数据判定，不按日历排期。这与 [ADR-0002](0002-retrieval-no-rag-lightweight-index.md) 的哲学一致：升级由数据触发，不时尚驱动。
>
...
事项围绕"每个机制是否可评估、每个升级决策是否有证据链"。多数为常态化动作，无完成态。
...
向量检索 / RAG 基础设施、ANN 索引、跨组织联邦协议——论证与重评触发条件见 [ADR-0002](0002-retrieval-no-rag-lightweight-index.md)。触发条件命中前不进任何阶段池。
