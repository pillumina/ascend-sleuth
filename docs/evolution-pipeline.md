...
### 6.3a 批提交模式（人审粒度 = 批，不打断自动化）

**用户诉求**：agent 应连续工作到目标完成，一次性提一个聚合 PR——每卡一个 PR 等人审会打断自动化流程。设计对齐仓库 knowledge-groom 先例（攒整个 inbox 出一份"变更摘要 + 待审项"给人一次审，不逐条 PR）。

**批的定义**：一轮会话（orchestration §1.2 的一轮）内产出的多张卡的改动，攒到轮末/批边界合成**一个聚合 PR** 送审。人审的单位是批不是卡。

| 级别 | 送审粒度 | 合入时机 |
|---|---|---|
| **auto** | 不送审（即时合入） | 每卡验证过即合入（对用户无感，不进批）——auto 本就无需人审，攒批只延迟观察窗无意义 |
| **review** | **攒批** | 卡实验通过 → 进入批内"待审"态；批边界触发 → 聚合 PR 一次人审 → 批合入 |
| **dual** | **攒批（标注高风险）** | 同上，聚合 PR 内该卡标 kb/high-risk，双签人重点审 |

**批边界（任一触发即提聚合 PR，防无限攒批）**：
- 轮次停止条件触发（§6.2/orchestration §2.2：预算耗尽/产出达标/收敛/人中断）——默认批边界 = 一轮；
- 攒批数量达上限（默认 10 卡，参数落地校准）——防单 PR 过大难审；
- **任务级攒批（可选）**：用户明确说"做到目标完成再提 PR"时，批边界延伸到长期任务的目标完成点（run §2 任务级）——此模式下 agent 跨多轮攒批，期间 review/dual 卡停留在"待审"态不进库，面板显示积压；**风险**：攒批越久，待审改动越晚合入、观察窗越晚开始、冲突风险越大，故任务级攒批须配"批内卡数/时间上限"双保险（上限触发即提前提批，不等目标完成）；**稳态/收敛同样提前触发提批**——某 scope 达 steady（orchestration §2.3）说明该方向已收敛，继续攒批只会积压无意义改动，应结算提批而非无限等目标。**任务级攒批 ≠ 无限攒批**：任何上限（卡数/时间/稳态）都强制提批，用户可续开新任务继续。
...
**批提交不改变验证门与观察窗**：每卡合入前仍需独立验证门（§6.5 无实验证据不合入——批内每卡都有各自实验记录，不是"批级一个实验覆盖所有"）；批合入后每卡独立进观察窗（execution §5）。批只改变"送审粒度"，不放松"验证强度"。
...
status: candidate                # 完整词表（v3，与 execution §5.1 一致）：
                                 ...
                                 #   in_experiment 数据通过 → pending_merge（批提交模式 §6.3a：攒批待审）
                                 #   pending_merge 批合入 → adopted；批内被打回 → re-iterate 或 rejected
                                 #   auto 卡不经过 pending_merge（实验通过即合入 → adopted）
...
candidate ──分级授权──► proposed ──实验启动──► in_experiment
    │                          │                   ├─ 数据通过（auto 卡）→ adopted（即时合入，进观察窗）
    │                          │                   ├─ 数据通过（review/dual 卡）→ pending_merge（攒批待审，§6.3a）
    │                          │                   │      ├─ 批合入 → adopted（进观察窗）
...
批提交模式下（§6.3a）：`pending_merge` 是 review/dual 卡实验通过后、批合入前的**攒批待审态**；auto 卡不经此态（即时合入）。`pending_merge` 不是滞留——批边界触发即提聚合 PR，卡随批合入或被打回。
