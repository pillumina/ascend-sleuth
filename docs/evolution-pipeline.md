...
4. **后续有验证确认改动合理、真解决问题**（合入后进观察窗，execution §5——validated/unconfirmed 如实标注，不是合入即算完）；
...
candidate ──分级授权──► proposed ──实验启动──► in_experiment
    │                          │                   ├─ 数据通过 → pending_merge（攒批待审，§6.3a；两种模式都经此态）
    │                          │                   │      ├─ 批合入 → adopted（进观察窗）
...
批提交模式下（§6.3a）：`pending_merge` 是卡实验通过后、批合入前的**攒批待审态**——**两种运行模式都经此态**（default 模式 auto 卡可即时合入跳过攒批，见 §6.3a 表；hands-off 模式全部卡攒批到任务完成）。`pending_merge` 不是滞留——批边界触发即提聚合 PR，卡随批合入或被打回。
