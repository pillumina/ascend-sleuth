...
- **活跃命名空间数量增长使优雅退化的索引成本逼近上下文预算**（按 $N_{\text{ns}} \times 30 \times 70$ token 估算；达 60 个命名空间即约 126K，需先做索引分片与分层 triage——选型排序见设计理论 §10.2）；
- Tier 3 语料超过 5000 篇，且 grep 兜底的挽救率可度量地不足（embedding 若引入，按设计理论 §10.1 从 Tier 3 假设生成层先于 Tier 2 候选过滤层）；
