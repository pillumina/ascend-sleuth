...
| 问 | 合的条件 | 拆的信号 |
|---|---|---|
| ① 同验证？ | 同一版本 pin + 同一次 `last_verified`（可以是同一份文档，也可以是一份能力的「配置面 + 执行面」文档） | 版本节奏不同、需分批重验 |
| ② 同诊断？ | 一次诊断会同时需要它们（同 category、同阶段） | 被不同 category 的诊断使用，且不会同次加载 |
| ③ 同动作？ | 是**同一次动作的三个面**（配置 / 执行 / 输出） | 是**两个独立动作**，各有自己的命令与输出面 |

**判据的实质**：三问合并起来只问一件事——它们是不是**一次动作的三个面**。是 → 合；是两个动作 → 拆。

**首用校准（2026-09-09，msprobe 首次应用）**：初版 ① 写作「同一份官方文档章节」，首用即暴露过严——`dump` 与 `config.json` 分属两份文档，却是「采集」这一次动作的配置面与执行面，按初版会被错拆；反之 `compare`（数值比对，输出 csv）与 `graph_visualize`（构图比对，输出 TensorBoard 图）虽同处"比对"阶段，却是两个独立动作、两套输出语义，应拆。故 ① 改为「同验证」、③ 改为「同动作」，并补「实质」一句。这是规则自身的第一次演化——判据由应用检验，不由设计臆定（原则十一）。
