...
**不做**：自动评估价值（读 body 判断）、自动沉淀（draft 前的判断环节留给 agent + 人）。**转正例外**：owner 预授权的自动化源产出的 draft（verification 链完整 + pre-triage 判别完成）可直接调 groom 升格，是预授权下的批量执行，非绕过人审（聚合 PR 仍人审合入）。
