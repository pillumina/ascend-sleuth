...
把上游 GitHub issue（如 vllm-ascend 的 closed bug）筛选、评估、沉淀为 case 的半自动管道。**自动化止步于"产 draft"，转正留人**（原则五：自动化产出建议，人做决定）——draft 进 inbox 批量审，知识生效点（draft → active）始终是人。
...
[标记] issue_filter.py --mark-imported <n1,n2>（追加 ingest-state.json 的 processed——幂等）
    ↓
[转正] 人批量审 inbox（groom 周批）→ draft → active
```
...
**不做**：自动评估价值（读 body 判断）、自动沉淀、自动转正——留给 agent + 人（判断性环节，违背自动化建议原则）。
