...
| 候选表里混进大量非文档，判定预算花在"判噪音"上 | `.txt` 在文档扩展名里、且**各仓自带的 agent 知识目录**此前未被排除。实测：加排除前全台账里 "agent 知识目录" 条目 186 条、构建/依赖清单（`CMakeLists.txt`/`requirements*.txt`）条目 491 条，合计 677 条（按新规则的 basename 口径复算为 678 条）而 **harvested 为 0**。已加进默认排除规则（`AGENT_KB_DIRS` + basename 规则）：agent 知识目录按"只作线索、不作权威源"的口径不入扫描面，构建/依赖清单按 basename 排除（不看目录层级） |
