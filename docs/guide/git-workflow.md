...
### 独立预核（提交前，由另一个 agent 做）

PR body 里的「Agent 预核意见」**由新上下文的 agent 产出**（看不到作者的推理），不是作者自己补一段自评。
...
| 改动类型 | 预核查什么 | 要跑反例吗 | 耗时 |
|---|---|---|---|
| 文档 / 措辞（`skills/**`、`docs/**`、`README`、`CLAUDE.md`） | ① 对照 `docs/spec/writing-norms.md` §1 的共用条目逐条过改动段落，报命中项 + 行号；② 文中每个**可验证声明**（路径 / 命令 / 字段名 / 代号 / 数字）逐个核对与实现是否一致；③ 改 skill 正文的另查 §6 的「内联完整共用条目」；④ 新增代号是否登记 `docs/glossary.yaml`、`scope` 是否对 | 不用 | 2–3 分钟 |
...
| 脚本 / 机制 / 生成物 / 门语义 | ① 对改动的判定逻辑**构造反例并跑出来**（给命令与结果）；② 检查门本身有没有洞；③ 前几轮的临时件有没有残留；④ 受影响的文档承诺扫一遍 | **必须** | 15–30 分钟 |

两条通用要求：

1. **没查出问题也要给覆盖清单**（查了什么、跑了什么命令、结果如何）——否则「没问题」与「没查」在 PR 上长得一样。
2. **每条结论带 `文件:行` + 一条可复跑命令 + 实测结果**；严重度用中文分级（阻断级 / 重要级 / 建议级），
