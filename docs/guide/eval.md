...
- **侧判据本身不在回放里测**：回放喂的是固定输入 + 固定 `framework`，负载类型由"框架名对到库里目录"
  这一档确定，所以夹具断言的是"**正确负载类型的知识被检索到 + 负载类型内性质判对**"，
  不是"负载类型是怎么判出来的"。后者只能由 trace 的 `workload_type` / `workload_type_source` 回答（早期 trace 里叫 `side` / `side_source`；当前还没有读端，将来接入时两种都取）——而这两个字段
