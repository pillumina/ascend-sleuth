...
- **机械可判**：某个 (侧 × 性质) 格子里库里有 case、而没有任何真实夹具钉住它 → 报告为缺口。
  CI 里只有 `--require-side training` 这一条（钉住"训练侧不许掉回零"）；其余格子的缺口
  用上面的命令现算现看，**不进 CI 结论**——补夹具需要真实来源，凭空造不出来，硬门化即假硬化。
- **夹具钉住词表的程度是**可读的**，而且差别很大（本次实测）**：`--nature-evidence` 用路由表
  自己的性质正则跑一遍夹具输入，26 条里只有 10 条**词法上**就命中期望性质（两条训练侧夹具都在
  其中），另外 16 条输入对三个性质零命中——它们靠的是流程里写明的语义兜底（`triage_semantic`）。
  这两种夹具的保护强度不是一回事：前者改坏词表会当场报（实测把 `RuntimeError` 从 interrupt
  挪到别的性质 → `MSLLM-1655` 报出），后者改坏词表也照样通过。
