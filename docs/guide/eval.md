...
- **硬门（CI `eval-scorecard`）**：①每条 fixture 必须入账本（新夹具没跑过就记 `not_recorded`，别空着）；②夹具字节哈希与账本不符 → 红——夹具就是量尺，被改过就该重跑一次；③`assertion` 要求命中（不含 `or-miss-documented`）而账本记着 miss → 红（这是夹具自述期望与观测的矛盾，放宽哪一种负载类型都要人决定）。过期由 `--build` 一条自动命令修，不需重跑回放。
...
### 按负载类型覆盖：路由分负载类型之后，"哪一种负载类型没有量尺"要能现算

路由从"一个分支同时记住哪一种负载类型与什么性质"拆成两层之后（负载类型由工程师的事实确定、性质在负载类型内判），
**负载类型成了量尺单位**：夹具的 `expected.namespace` 第一段就是负载类型。此前这件事只能靠人肉数，
而读出来的结论容易被误读——`eval/holdout.py --list` 报的"有 case 却没有夹具的格子"是
(namespace × 性质) 的结果格子，读不出"训练侧 0 条"（量尺单位）。数字一律现算，别在文档里写死。

```
python3 scripts/eval_workload_type_coverage.py                  # 按负载类型覆盖报告（人读）
python3 scripts/eval_workload_type_coverage.py --check          # 负载类型层：某一种负载类型量尺归零即红
python3 scripts/eval_workload_type_coverage.py --check-cells    # 格层：任一格缺夹具也当门（默认只报）
python3 scripts/eval_workload_type_coverage.py --check --require-workload-type training   # 只要求点名的负载类型不归零
python3 scripts/eval_workload_type_coverage.py --nature-evidence                 # 附加：哪几条夹具钉着词表
```
...
**覆盖格子是 (负载类型 × 性质)、不含框架**：同一负载类型同一性质下的所有框架共用一格（`training/<任意框架>/interrupt`
算同一格），所以 `inference 22/124` 不是"逐框架覆盖"。`common/` 是框架无关的共享池，
不判负载类型，单列不计入缺口。
`--root` 指错地方时报读取错误（退出 2），**不静默报绿**——`glob` 对不存在的根返回空，
不拦就会把"根够不着"印成"每种负载类型 0 条 case ✓"，那正是这份报告要消灭的那类误读。

**强度分层（如实标注，同 holdout 的写法）——两层判据，两个开关各管一层**：

- **负载类型层（`--check`，CI 跑的就是这一层）**：库里**有 case 的**某一种负载类型**一条真实夹具都不剩** → 红。
  它防的是"整条负载类型量尺归零"。`--require-workload-type <负载类型>` 把这一层限定到点名的负载类型。
  **只对"库里有 case 的负载类型"要求夹具**：新负载类型先在后端声明、下一条 PR 才补 case 与夹具是正常顺序
  （路由协议文件写明负载类型列表可扩张），那时没有"需要保护的东西"，不该被拦。
- **格层（`--check-cells`，默认只报不拦）**：某个 (负载类型 × 性质) 格子有 case、没有任何真实夹具 → 红。
...
- **它测不到的东西**：夹具钉的是**某一条真实输入赖以命中的那几个正则**，不是那一种负载类型的整份词表。
  实测把训练侧的性质词从一处挪到另一处（`build_train_valid_test_data`、`recv_forward`、
  `重复注册` 等训练专属 token，以及 `日志.*异常`）**语料级与夹具级都零检出**——这些词不是任何
  夹具输入的唯一证据，别的正则仍先接住它。所以"训练侧有夹具了"只读作"该负载类型链路接得通、
  且那几条 case 的性质判对了"，**不读作"训练侧词表被守住了"**。词表层仍靠人审
  （路由表里 `needs-review` 那条标法）。
- **负载类型判据本身不在回放里测**：回放喂的是固定输入 + 固定 `framework`，负载类型由"框架名对到库里目录"
  这一档确定，所以夹具断言的是"**正确负载类型的知识被检索到 + 负载类型内性质判对**"，
  不是"负载类型是怎么判出来的"。后者只能由 trace 的 `workload_type` / `workload_type_source` 回答（早期 trace 里叫 `side` / `side_source`，两种都认）——而这两个字段
...
performance 与 precision——这两类按设计就不靠报错字面量判别。故方向在**内容负载类型**：审计「工程师会粘贴什么」
...
**不做批量补齐**——审计显示输入负载类型字面量以环境模板噪声（版本号/OS/CPU/环境变量清单）为主，补进去是稀释。
