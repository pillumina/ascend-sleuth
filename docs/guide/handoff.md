...
**产出侧**：路径是确定的（`<traces>/exports/<session_id>/` 与 `handoff-<session_id>.{zip,md}`），
再导一次**覆盖同一个位置**，不会堆出 `-1`、`-2` 副本。导出前会先清掉上一次的产出树，所以从源里撤掉的
证据文件不会残留在包里；落盘后还会校验"交接单清单 == 树"，不一致就直接失败（不自相矛盾的包发出去）。
清旧树只对**认得出来的**上一次产出做（含 `<session_id>.yaml` 或 `handoff/<session_id>.yaml`）；
认出不来就拒收且不动那个目录——`--out` 可以指到任何地方，不能对它删目录。

两次导出**不是逐字节相同**：交接单的 `exported_at` 会变，zip 里也带文件时间。要"同一份包"就以手上那份
文件为准，别指望重导一次得到同样的字节。

**接收侧按"这份包"认重，不按会话认**：`exported_at` 与源 `session_id` 都对上才判为同一份，不重复落位；
两份**不同的导出**各算一单，第二份落成 `<session_id>-imported-<n>`——它们可能内容一致，也可能不同
（上家在两轮之间补过证据），要不要合并是判断，脚本不替人决定。

"已经导出过了"不阻断再导一次：导出只读源、只写 `traces/exports/`，重导就是刷新那个位置的内容。
