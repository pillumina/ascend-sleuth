...
| `scripts/audit_skill_cost.py` | **skill 上下文成本审计**：常驻面（CLAUDE.md/AGENTS.md/被注入的 description）、按需面（正文 + 本地 references）、重复面（同一行出现在 ≥2 skill）、强制词密度与输出段数；skill-review 的 A 静态审计调它。`--check` 是**固定面基线比对（只降不升）**：比对上 `metrics/gates.yaml` 的 `token_budget.baseline`，三态退出码（0 两项都在基线内 / 1 有一项超基线 / 2 判据没被评估），读法同 `metrics_health.py --check` | skill 改动前后；内容流程收尾 |
...
| 固定面（token） | `token_budget.baseline` + `tolerance_ratio` | 由 `audit_skill_cost.py --check` 判，不走 `metrics_health.py`：它比的是**文件字节**，与 timeline 无关。基线只允许下降；放大要改基线并在 PR 写明理由。**为什么用「只降不升」而不是绝对阈值**：这两项不买判别力（索引行内上限从 12 提到 30 再放开，四种改法的召回都是 14/25），所以只需要一个方向；绝对上限要多期数据才敢定 |

**token 这一维的读数在哪**：固定面看 `audit_skill_cost.py --check`；逐个会话的真实账单看
`node scripts/session_cost.mjs`（`--all` 汇总历史、`--steps` 出逐步表：上下文增量 × 工具与返回大小）。
**两把尺不要混**：基线比对量的是文件字节（≈tok = B/3.4，估算），账单量的是提供方 usage。
