...
**token 这一维的读数在哪**：逐个会话的真实账单看 `node scripts/session_cost.mjs`——默认给按轮汇总与最大的 5 步；
`--steps` 给逐步表（该步上下文规模与新增 × 这一步调了什么工具、返回多大）；`--tools` 给工具流量
（调用次数、返回累计、单条上限、重复调用与重复目标）。固定面的静态成本看 `scripts/audit_skill_cost.py`。
**两把尺不要混**：前者是提供方 usage（计费），后者是文件字节按 B/3.4 折算（估算）。

实测（355 个会话）：账单里缓存重读占 98.9%、未缓存输入占 0.72%，花费的主项是**步数 × 每步上下文规模**；
最大的 5 个会话平均每步上下文 378K–485K。所以"文档瘦 5%"在账单上几乎看不见，要省得从步数与每步上下文入手。
