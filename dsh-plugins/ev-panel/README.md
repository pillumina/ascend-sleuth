...
**规范本体在 [../README.md](../README.md)「文案规范」节**（两个面板共用的唯一权威处，条目与反例
都在那边，条数不在这里复写）。本面板是那套规范的主要来源：它原先的文案以叙述散文为主，AI 味与读不懂都从那里来，
所以规范里多数反例取自本面板的早期版本。

本面板相关的两条补充：

- **判据标题与口径是数据**（`proposals/gates.yaml` 的 `title` / `meaning` / `action`），
  带数值的读数模板在 `scripts/evolution_health.py`。改文案不必动代码；但改了要同步改
  `panel_render_check` 里钉它的断言（实测一次措辞重写牵连 18 条断言）。
- **内部状态码不上屏**：实测踩过把 `state: ok` 当人话打印成"记录：ok"，现映射为中文
  （见 `LEDGER_LABEL`）。同类还有枚举名与 metric id。
