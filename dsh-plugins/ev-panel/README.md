...
| ④ 执行现场（exec-log） | **每次内容流程收尾，evolve-check 到底跑没跑** | `ExecLogSection`：收尾次数 / 其中无信号次数 / 最近执行的 skill·产出·理由；**同一克隆共享**（主检出 `metrics/skill-exec-log.yaml`，跨 worktree 共写共读；跨克隆/跨机不聚合，区块内显式标注） |
