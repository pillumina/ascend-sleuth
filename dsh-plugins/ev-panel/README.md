...
让"看到系统在自演进"成为可能（run.md §6 领域视图落地：卡流转为主，容量/台账/指标为辅）。
...
- **流程演进信号**：组件失败台账（mis 侧）+ S2 replay 路由归因（S1 无关）——**无数据显示"数据积累中"，不画假图**（诚实退化）
- **指标趋势**：timeline live 期趋势（routed_accuracy）

## 数据流

```
scripts/ev_board_data.py  ← 确定性聚合（EV 卡/容量/台账/timeline → JSON，原则二）
   ↑ shell 调用
