...
| 定位 | 卡片的「定位」只给**真 case id**；`active_case` 里写的是"没命中"的占位串（`pending-investigation` 系）时按「未定位到知识库 case」显示，并把 trace 里写的原值摆出来说明它不是 case id——占位串当 case 显示会连带生成"该 case 的 fix 生效了吗"这类指令 |
