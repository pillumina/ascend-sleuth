...
| 报告与沉淀入口 | 卡片给「看报告」（面板内读：章节跳转 + 只渲染所选一节 + 复制全文 + 打开文件）、「打开报告」（外部编辑器）与「待沉淀 N 条」（trace `sediment_candidates` 条数）；报告名认两种来源：trace 记录的 `report_file`（**顶层与 `report` 事件里都认**）与 `<trace 同名>.report.md` 规则，走后者时卡片上写明来源；面板只读，不改报告内容 |
