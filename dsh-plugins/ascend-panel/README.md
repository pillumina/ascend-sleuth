...
| 看到的不等于现实 | 面板读的是**生成物**索引/快照（条数与逐格容量**现算**：`index_counts.py`／体检链；索引头注已不再写数字，写进去就会在并发合并时撞行或漂移），陈旧时不标注 | 状态条报快照龄期（`2026-W37-live · 1 天前（阈值 7 天）`）+ 索引↔磁盘 drift（`索引 N ≠ 磁盘 M`；两者一致时不显示告警） |
