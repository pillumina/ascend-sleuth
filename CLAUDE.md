...
- **对话回复写人话**：回复用户时先写中文含义，内部代号放括号里。代号指卡号、事项号、机制名、分组名这类读过本仓文档才懂的简称。必须照抄的原值不在此列（原始字段值与枚举、仓库内路径与命令、报错原文、`文件:行号`、编号与数字、算子名），改掉就无法核对；完整清单见 `docs/spec/writing-norms.md` §2。标准是读者只看这段回复就能懂，不必先去读 `docs/`。
