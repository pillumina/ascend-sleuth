...
- **生成物的两个提交面（并发提交不撞的关键）**：**人提交面** = case 本体 + 索引分片 `knowledge/_index/<ns>__<category>.yaml` + 路由源 `triage-tree.d/<族>.yaml`（都在 PR 里，有门守着）；**主干重建面** = 索引总表 `knowledge/_index.yaml` 与路由聚合 `triage-tree.yaml`（由 CI 的 generated-refresh 在合并后重建并提交，「生成物不得进 PR」那道门不让它们出现在 PR 里）。为什么：那两张表是"谁改 case 都得重写一遍"的全库/全路由大表，三个人并发就撞在同一个文件上；分片与族文件按 (框架 × 性质) / 分支族切开，改不同框架的人互不相碰。命令：改 case 后 `scripts/build_index.py`（提交分片）、加路由词改族文件后 `scripts/build_triage_tree.py`。Retrieval is deliberately lexical/structural — no vector RAG (see `docs/adr/0002`).
