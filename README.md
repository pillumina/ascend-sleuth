...
- **框架式**：团队 fork 本仓库后自行积累或导入知识。上游只同步方法论与机制账本目录（`skills/ scripts/ docs/ examples/ eval/ .github/ proposals/ideas/`），知识目录不参与上游合并。两侧各自独占的目录、每部署一份的共享文件与生成物的处理纪律见 [docs/guide/git-workflow.md](docs/guide/git-workflow.md) 的「目录归属」一节。
