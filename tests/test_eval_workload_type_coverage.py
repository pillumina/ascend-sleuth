"""eval_workload_type_coverage（回放夹具的按负载类型覆盖报告）的口径回归测试。

为什么值得测：这份报告要支撑的结论是"某一种负载类型没有量尺"——它一旦算错，两个方向都不轻：
把构造示例算成覆盖 → 缺口被掩盖（本次要修的就是这个误读）；把真实夹具漏算 →
报了不存在的缺口，人就去补已经有的东西。所以钉四件事：真实夹具怎么算、
构造示例与指向未入库 case 的夹具怎么排除、`--check` 的两档（任一缺口 / 指定侧必须有）、
以及"按负载类型覆盖"这个数与格子表对得上。
"""
...
import eval_workload_type_coverage as ESC  # noqa: E402
...
class EvalSideCoverageTest(unittest.TestCase):
    ...
    def seed_both_workload_types_covered(self):
        """两侧各一条 case + 一条夹具——`--check` 判**全部已声明的负载类型**，所以格层用例要先满足负载类型层。"""
    ...
    def run_cli(self, *args):
        return subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "eval_workload_type_coverage.py"),
             "--root", str(self.root), *args],
            capture_output=True, text=True)

    # ---------------------------------------------------------------- 库侧口径
    def test_library_cases_bucket_by_workload_type_and_nature(self):
        self.write_case("training/mindspeed-llm/interrupt/T-1.yaml", "T-1")
    ...
    def test_common_pool_is_not_a_workload_type(self):
        """common/ 是框架无关的共享池——它不判负载类型，所以既不算进某一种负载类型，也不算缺口。"""
    ...
    def test_workload_type_and_nature_parsed_from_namespace(self):
        self.write_fixture("T-9.fixture.yaml", "T-9", "inference/vllm-ascend/precision/")
        row = ESC.fixture_rows(self.root)[0]
        self.assertEqual((row["workload_type"], row["nature"]), ("inference", "precision"))

    # ---------------------------------------------------------------- 门的两档
    # ------------------------------------------- 两层判据的分界（这是本脚本最容易做错的地方）
    # 负载类型层（`--check`）管"某一种负载类型一条真实夹具都不剩"；格层（`--check-cells`）管"某个
    # (侧 × 性质) 格子有 case 没夹具"。**两层必须能各自独立触发**——本脚本第一版把两层
    # 压在一个 `--check` 里，于是文档写的门与实际跑的门不是同一个，下面四条钉住这个分界。
    def test_workload_type_layer_red_when_a_type_has_no_fixture(self):
        """某个负载类型有 case 却一条夹具都没有 → 负载类型层红。"""
        self.write_case("training/mindspeed-llm/interrupt/T-1.yaml", "T-1")
        r = self.run_cli("--check")
        self.assertEqual(r.returncode, 1)
        self.assertIn("负载类型层", r.stdout)
    ...
    def test_cell_layer_is_report_only_by_default(self):
        ...
        """
        self.seed_both_workload_types_covered()
    ...
    def test_workload_type_layer_ignores_types_without_cases(self):
        """负载类型层只对**库里有 case 的负载类型**要求量尺。
        ...
        self.write_case("training/mindspeed-llm/interrupt/T-1.yaml", "T-1")
        self.write_fixture("T-1.fixture.yaml", "T-1", "training/mindspeed-llm/interrupt/")
        # inference 此时既无 case 也无夹具：不该被负载类型层当成缺口
        self.assertEqual(self.run_cli("--check").returncode, 0)
        # 但一旦落了 case，就必须有夹具
        self.write_case("inference/vllm-ascend/interrupt/I-1.yaml", "I-1")
    ...
    def test_check_green_when_both_layers_are_clean(self):
        ...
        self.assertEqual(r.returncode, 0)
        self.assertIn("负载类型层：每个已声明的负载类型都有真实夹具", r.stdout)
        self.assertNotIn("默认只报不拦", r.stdout)

    def test_require_workload_type_limits_the_workload_type_layer(self):
        """`--require-workload-type` 把负载类型层判据限定到点名的那几种——没点名的就算归零也不拦。"""
        self.write_case("training/mindspeed-llm/interrupt/T-1.yaml", "T-1")
        self.write_fixture("T-1.fixture.yaml", "T-1", "training/mindspeed-llm/interrupt/")
        # inference 侧这时**有 case、无夹具**：这才是"归零"该判的场景（没 case 的侧已被跳过）
        self.write_case("inference/vllm-ascend/interrupt/I-1.yaml", "I-1")
        self.assertEqual(self.run_cli("--check").returncode, 1)
        self.assertEqual(self.run_cli("--check", "--require-workload-type", "training").returncode, 0)
        self.assertEqual(self.run_cli("--check", "--require-workload-type", "training",
                                      "--require-workload-type", "inference").returncode, 1)

    def test_require_workload_type_rejects_unknown_workload_type(self):
        """写错的侧名要报出来，不能静默当成"通过了"——那是这个脚本最容易骗自己的地方。"""
        r = self.run_cli("--check", "--require-workload-type", "trainig")
        self.assertEqual(r.returncode, 2)
        self.assertIn("trainig", r.stderr)

    def test_require_workload_type_green_when_that_workload_type_is_covered(self):
        self.write_case("training/mindspeed-llm/interrupt/T-1.yaml", "T-1")
        self.write_fixture("T-1.fixture.yaml", "T-1", "training/mindspeed-llm/interrupt/")
        r = self.run_cli("--check", "--require-workload-type", "training")
        self.assertEqual(r.returncode, 0)
    ...
    def test_missing_root_is_read_error_not_green(self):
        ...
        `glob` 对不存在的根返回空，报告会把"根够不着"印成"库里没有 case、每种负载类型 0/0"加一句 ✓——
...
"""
    ...
    def test_nature_evidence_denominator_excludes_natureless_namespaces(self):
        ...
        self.assertEqual(rows[0]["workload_type"], "inference")
    ...
    # ---------------------------------------------------------------- 本仓实况：缺口不再被掩盖
    def test_repo_reports_training_workload_type_as_covered(self):
    ...
    def test_repo_training_fixtures_are_lexically_anchored(self):
        ...
        train = {e["fixture"]: e for e in ev if e["workload_type"] == "training"}
    ...
    def test_repo_nature_evidence_report_is_self_explanatory(self):
        ...
        """
        r = subprocess.run([sys.executable, str(ROOT / "scripts" / "eval_workload_type_coverage.py"),
