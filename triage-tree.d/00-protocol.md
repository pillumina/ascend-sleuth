...
加词/改词/改分支的入场判据（判据本身见 scripts/trace_metrics.py 的 triage_miss_classes）：
  ...
  **「没见过这种写法」不是证据**——多语种与换词属语义面，补词不收敛（补的是措辞，不是能力）。
  ...
  配置名）只在训练（或推理）的输入文本里出现，留在共用词表不会误吸另一侧的 case；
...
workload_types:                         # 负载类型层：合法负载类型、各自的目录面与检索顺序（负载类型的单一来源是工程师的事实）
  - id: training
    label: 训练
