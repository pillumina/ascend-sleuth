...
#
# S2 = Issue-replay 校准（docs/mechanism/pipeline.md §2.1 / docs/mechanism/run.md §3）：
...
def main():
    ...
    if args.incremental and args.output.exists():
        try:
            ...
            pass
    doc = {
        "_comment": f"GENERATED by scripts/s2_calibration.py（{args.repo}，S2 issue-replay 校准集，见 docs/mechanism/run.md §3）。split=test 的条目仅用于 validated 终判，不参与 gate 决策；expected.resolution 是 issue 实际 resolution（外部 ground truth）。",
        "calibration": merged,
    }
