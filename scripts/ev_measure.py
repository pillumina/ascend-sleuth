...
#
# 实测记录（2026-09 新增，判据「声明了却没测」的数据源）：
#   原先 `--run` 只往 stdout 打印，跑没跑过在数据上完全不可区分——于是"9 张卡写了可复现判据、
#   一次都没被复现过"这件事**没有任何观测面**（与 exec-log 修前的结构缺陷同形：声明了 ≠ 测过）。
#   现在每次 `--run` 往**同一克隆共享**的 metrics/ev-measure-log.yaml append 一笔
#   （路径与写锁复用 exec_log_path，非 worktree 本地件）。本记录不改变判定强弱：
#   它只证明"这条判据被执行过"，不证明"命令真在测那件事"（后者仍是约定强度）。
#
# 用法：
#   python3 scripts/ev_measure.py EV-2026-050            # 只看判据（不执行）
#   python3 scripts/ev_measure.py EV-2026-050 --run      # 执行并比对（并落一笔实测记录）
...
# 导入时必须取别名，否则后者被遮蔽（实测：实测记录已写入却报"写入失败"）。
from log_skill_exec import git_head  # noqa: E402
...
def append_run(root: Path, card_id: str, command: str, verdict: str, exit_code: int,
               explicit_log: Path = None, local: bool = False) -> str:
    """往共享运行件 append 一笔实测记录 → 返回人读落点描述（写失败也返回，不抛）。

    为什么共享而不是检出内：与 exec-log 同因（面板读主检出；worktree 清理不丢数据）。
    写侧持锁：read-modify-write 并发会互相覆盖。无 flock 的平台如实提示（不假装有互斥）。
    """
    path, where = resolve_rel(root, MEASURE_LOG_REL, explicit=explicit_log, local=local)
    try:
        path.parent.mkdir(parents=True, exist_ok=True)
        with log_lock(path) as locked:
            records = []
            if path.exists():
                try:
                    ...
                    # 坏文件：先把原文件另存再重开——「如实重开」不等于可以静默删掉读不出来的历史
                    try:
                        path.replace(path.with_name(
                            f"{path.name}.bad-{datetime.now().strftime('%Y%m%d%H%M%S')}"))
                    except Exception:
                        pass
                    records = []
            entry = {
                ...
                "verdict": verdict,
                "exit_code": exit_code,
                "command": one_line(command, 160),
                "version": git_head(root),
            }
            records.append(entry)
            comment = ("GENERATED by scripts/ev_measure.py --run；EV 卡预测的实测记录（append-only）。"
                       "路径由 scripts/exec_log_path.py 解析到**主检出**（同一克隆共享，worktree 清理不丢），"
                       "故「声明了可复现判据却从没被测过」在数据上可区分。")
            # 用 yaml.safe_dump 而不是手工拼字符串：measure.command 是任意命令行，含单引号时
            # `command: '...'` 会产出非法 YAML → 下一次 load_file 抛错 → records 为空重开，
            # 于是「append-only」的记录被静默清空（实测：连续两次 --run 后文件只剩最后一笔且解析失败）。
            body = yaml.safe_dump({"records": records}, allow_unicode=True, sort_keys=False,
                                  default_flow_style=False, width=10 ** 6)
            path.write_text(f"# {comment}\n\n{body}", encoding="utf-8")
        note = describe_log_path(path, where)
        if not locked:
            note += " ⚠ 本平台无 fcntl.flock，并发 append 无法互斥"
        return note
    except Exception as e:
        return f"（实测记录写入失败，不影响本次判定：{e}）"
...
def cmd_card(root: Path, args, card_id: str) -> int:
    ...
    code = {"PASS": 0, "FAIL": 1, "ERROR": 2}[verdict]
    note = append_run(root, card_id, measure["command"].strip(), verdict, code,
                      explicit_log=args.log, local=args.local)
    print(f"实测记录: {note}")
    if verdict == "PASS":
...
def main() -> int:
    ...
    ap.add_argument("--log", type=Path, default=None,
                    help="显式指定实测记录路径（默认：同一克隆共享的主检出 metrics/ev-measure-log.yaml）")
    ap.add_argument("--local", action="store_true",
                    help="实测记录写在当前检出内（用于隔离测试/演练，不参与共享）")
