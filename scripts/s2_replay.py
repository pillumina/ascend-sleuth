...
def collect(root: Path, report_path: Path):
    ...
    for f in sorted((root / S2_DIR).glob("*.yaml")):
        ...
        for e in doc.get("calibration", []):
            ...
            if route_miss and not got_hit:
                # 路由 miss 事实记录（候选归因，非 execution_error 定论）：route miss 可能是
                ...
                attribution_entries.append({
                    ...
                    "verdict": "candidate",           # 候选——路由 miss 事实，组件归因待确认
    ...
    # 写路由 miss 记录（供 component_tally.py 按需聚合；无 miss 则写空文件不报错）
    attr_path = root / REPLAY_DIR / "attributions.yaml"
    attr_path.parent.mkdir(parents=True, exist_ok=True)
    attr_path.write_text(
        yaml.safe_dump({
            "_comment": "GENERATED by scripts/s2_replay.py --collect；S2 路由 miss 记录（S1 无关）。"
                        "component=got 分支（候选指向，非定论），verdict=candidate——S2 只能报 miss 事实，"
                        "组件归因需人/trace 确认。由 component_tally.py 按需聚合（无常驻台账表）。",
            "attributions": attribution_entries,
        }, allow_unicode=True, sort_keys=False), encoding="utf-8")
    if attribution_entries:
        print(f"s2_replay: {len(attribution_entries)} 条路由 miss 候选 → {attr_path}（component_tally 按需聚合）")
