...
#   - **侧层（`--check`）**：库里**有 case 的**某一侧一条真实夹具都不剩 → 退出 1。
#     CI 跑的就是这一层。它防的是"整侧量尺归零"。只对"有 case 的侧"要求夹具——
#     新侧先声明、下一条 PR 才补 case 与夹具是正常顺序，那时没有需要保护的东西。
...
def main() -> int:
    ...
    for side in (args.require_side or list(SIDES)):
        # 只对**库里有 case 的侧**要求量尺：侧层的判据是"这一侧没有回归保护"，
        # 而"新侧先在后端声明、下一条 PR 才补 case 与夹具"是正常顺序（路由协议文件里
        # 写明侧列表可扩张），那时拦下来是拦错了对象。
        if not any(k[0] == side for k in lib):
            continue
